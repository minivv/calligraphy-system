<template>
  <div>
    <div class="detail-card" v-if="activity">
      ...
      <div class="participants-info">参与人数：{{ activity.currentParticipants }} / {{ activity.maxParticipants }}</div>
      ...
      <div class="action-bar">
        ...
        <el-button :type="likeStatus===1?'primary':'default'" @click="doLike(1)"><i class="el-icon-thumb"></i> 点赞 ({{ activity.likeCount }})</el-button>
        <el-button :type="likeStatus===2?'danger':'default'" @click="doLike(2)"><i class="el-icon-thumb" style="transform:rotate(180deg);display:inline-block"></i> 踩 ({{ activity.dislikeCount }})</el-button>
...
<script>
export default {
  data() { return { activity: null, likeStatus: 0, comments: [], commentTotal: 0, commentContent: '', complaintVisible: false, complaintForm: { title: '', content: '' } } },
  computed: { isLogin() { return !!localStorage.getItem('user') } },
  created() { this.loadActivity(); this.loadComments(1) },
  methods: {
    async loadActivity() {
      const res = await this.$request.get('/api/front/activity/' + this.$route.params.id); this.activity = res.data
      if (this.isLogin) { try { const lr = await this.$request.get('/api/like/status', { params: { targetType: 'activity', targetId: this.activity.id } }); this.likeStatus = lr.data ? lr.data.type : 0 } catch (e) {} }
    },
    async loadComments(page) { const res = await this.$request.get('/api/front/comment/page', { params: { targetType: 'activity', targetId: this.$route.params.id, pageNum: page || 1 } }); this.comments = res.data.records; this.commentTotal = res.data.total },
    async doRegister() { if (!this.isLogin) return this.$message.warning('请先登录'); try { await this.$request.post('/api/registration', { activityId: this.activity.id, remark: '' }); this.$message.success('报名成功，等待审核') } catch (e) {} },
    async doLike(type) { if (!this.isLogin) return this.$message.warning('请先登录'); await this.$request.post('/api/like', { targetType: 'activity', targetId: this.activity.id, type }); this.loadActivity() },
    async submitComment() { if (!this.commentContent.trim()) return this.$message.warning('请输入评论内容'); await this.$request.post('/api/comment', { targetType: 'activity', targetId: this.activity.id, content: this.commentContent }); this.commentContent = ''; this.loadActivity(); this.loadComments(1) },
    ...
    async submitComplaint() { await this.$request.post('/api/complaint', { ...this.complaintForm, targetType: 'activity', targetId: this.activity.id }); this.$message.success('投诉已提交'); this.complaintVisible = false }
  }
}
</script>
...
.participants-info { margin-top: 12px; font-size: 14px; color: var(--color-text-secondary); font-weight: 500; }
.detail-divider { height: 1px; background: var(--color-border); margin: 24px 0; }
.detail-image { max-width: 100%; max-height: 400px; border-radius: var(--radius-input); }
